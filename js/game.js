...
class game {
  ...
  drawText() {
    ...
    if (this.score > tager) this.context.fillStyle = "#FF6600";
  ...
  initGold() {
    ...
    while (true) {
      ...
      for (let i = 0; i < N - 1; i++)
        for (let j = i + 1; j < N; j++)
          while (
            this.range(this.gg[i].x, this.gg[i].y, this.gg[j].x, this.gg[j].y) <
            2 * this.getWidth()
          ) {
            check = false;
            this.gg[j].randomXY();
...
const loader_bar_background = document.querySelector(".loading-bar-background");
const loader = document.querySelector(".loader");
loading_text.style.display = "none";
loader_bar_background.style.display = "none";

const playgame = () => {
  loading_text.style.display = "block";
  loader_bar_background.style.display = "block";
